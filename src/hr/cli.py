...
def create_parser():
    ...
    parser.add_argument('--path','-v',help='the path to the export file')
...
def main():
    ...
    # Here we're going to create a parser, and immediately have it start parsing the args,
    # so that we have access  to the args.
    args = create_parser().parse_args()

    # This reads in the user information (from the pwd module that we used in users.py).
    users = u.fetch_users()

    # Now for some conditional logic, based on the path that's  passed in, to determine the file that we're using
    #and the format that's passed in, so that we know how to export it

    # The first argument is going to be the path.
    if args.path:

        # If the path is present at all, we're going to open the file,
        # make sure it's writable, and set the newline to be an empty string
        # We don't want any extra strings or characters in what we're doing.
        file = open(args.path, 'w', newline='')

    # If we're NOT writing to a file, then the output is going to be STDOUT
    else:
        file = sys.stdout
    ...
    # export module to_json_file and write users out to the file in JSON format.
    if args.format == 'json':
        export.to_json_file(file, users)

    # And if it's not JSON, then we're planning on it being csv.
    #We'll call the export module to_csv_file and write out users to the  file in csv format.
    else:
