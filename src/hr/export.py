...
# a function called to_json_file. It will take the user data from the fetch_users function
...
def to_csv_file(export_file, users):
     ...
     # Once the line is done writing, we need to go to the next line and start the actual exporting of the data
     export_file.write('name,id,home,shell\n')

     # Now is where we're actually starting to use the csv package.
     # We'll create a writer that will do the actual writing out of the data.
     writer = csv.writer(export_file)

     # We already wrote the header line to the file, and
     # since it ended with a newline character our rows will all go below the header.
